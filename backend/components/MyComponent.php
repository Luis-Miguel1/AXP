...
class MyComponent extends Component
{
  ...
  /**
  * @test Função para notificar acção bem sucessedida nesse caso Apagar
  */
  public function flashMessage()
  {

    if(Yii::$app->session->hasFlash('sucess')){
        $type = 'sucess';
        $alertType = 'success';
        $message = 'Apagado';
    }elseif (Yii::$app->session->hasFlash('recovery')) {
        $type = 'recovery';
        $alertType = 'info';
        $message = 'Recuperado';
    }elseif (Yii::$app->session->hasFlash('publish')) {
        $type = 'publish';
        $alertType = 'success';
        $message = 'Publicado';
    }elseif (Yii::$app->session->hasFlash('unpublish')) {
        $type = 'unpublish';
        $alertType = 'warning';
    ...
    if (Yii::$app->session->hasFlash($type)): ?>
          <div class="alert alert-<?= $alertType ?> alert-dismissable">
              ...
              <h4><i class="icon fa fa-check-square"></i> <?= $message ?>!</h4>
  ...
  //funtion to redirect llink from database and convert to correct link
  public function playAudio($model)
  {
    return str_replace('/rtc','',Url::base()).'/'.str_replace('../../','',$model->link_audio);
  }

  //funtion to redirect llink from database and convert to correct link
  public function playVideo($model)
  {
    return str_replace('/rtc','',Url::base()).'/'.str_replace('../../','',$model->link_video);
  }
